...
public:
   ...
   */
  inline iterator
  longest_prefix_match (const FullKey &key, ns3::Ptr<const Exclusion> exclusionFilter = NULL, bool disableRanking = true, int count = -1, Ptr<Face> inFace = NULL)
  {
    iterator foundItem, lastItem;
    bool reachLast;
    boost::tie (foundItem, reachLast, lastItem) = trie_.find (key, exclusionFilter, disableRanking, count, inFace);
  ...
  inline iterator
  longest_prefix_match_if (const FullKey &key, Predicate pred, ns3::Ptr<const Exclusion> exclusionFilter = NULL, bool disableRanking = true, int count = -1, Ptr<Face> inFace = NULL)
  {
    iterator foundItem, lastItem;
    bool reachLast;
    boost::tie (foundItem, reachLast, lastItem) = trie_.find_if (key, pred, exclusionFilter, disableRanking, count, inFace);
    if (foundItem != trie_.end ())
      {
        policy_.lookup (s_iterator_to (foundItem));
      }
   ...
   */
  inline iterator
  deepest_prefix_match (const FullKey &key, ns3::Ptr<const Exclusion> exclusionFilter = NULL, bool disableRanking = true, int count = -1, Ptr<Face> inFace = NULL)
  {
    iterator foundItem, lastItem;
    bool reachLast;
    boost::tie (foundItem, reachLast, lastItem) = trie_.find (key, exclusionFilter, disableRanking, count, inFace);
  ...
  template<class Predicate>
  inline iterator
  deepest_prefix_match (const FullKey &key, Predicate pred, ns3::Ptr<const Exclusion> exclusionFilter = NULL, bool disableRanking = true, int count = -1, Ptr<Face> inFace = NULL)
  {
    iterator foundItem, lastItem;
    bool reachLast;
    boost::tie (foundItem, reachLast, lastItem) = trie_.find (key, exclusionFilter, disableRanking, count, inFace);
